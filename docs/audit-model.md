...
### Correlation ID at gateway ingress

Correlation ID assignment is handled by `CorrelationMiddleware`
...
**Coverage of pre-evaluation failure paths:**
`X-Correlation-ID` is returned on all gateway responses. Responses that fail before
...
**Audit coverage of pre-evaluation failure paths:**
Gateway-level `AuditEvent` records are emitted for every significant outcome that
...
invoked is preceded by an `emit_gateway_event` call. Covered paths: request body
validation failure, Bearer token extraction failure, JWT verification failure, subject
mapping failure, and evaluator unavailability. The fail-closed probe path
(`AUDIT_FAIL_CLOSED=true`, writer degraded) attempts an emit before returning 503; the
emit fails because the writer is broken, which is inherent — there is no working writer
to record to.

One latent gap: the `RequestValidationError` exception handler in `main.py` returns a
`validation_failed` 400 with no `emit_gateway_event`. It is currently unreachable for
`/v1/evaluate` because the route reads its body manually (`await request.body()`) and
its only FastAPI-managed parameter is `authorization: str | None = Header(default=None)`,
which never fails validation. If the route signature ever gains a typed FastAPI-managed
body or query parameter, requests failing at that layer would exit without audit evidence.
